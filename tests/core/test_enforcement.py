"""Tests for the 3-step enforcement chain."""
...
def test_posture_unclassified_with_default() -> None:
    assert (
        check_posture(None, Posture.READ_WRITE, Posture.READ_ONLY).verdict
        == EnforcementVerdict.ALLOW
    )
...
def test_enforce_override_allow_does_not_bypass_posture_ceiling() -> None:
    tool = ResolvedTool(
        name="special", server_name="fs", family="fs", posture=Posture.DESTRUCTIVE
    )
    ...
    result = enforce("special", tool, profile, ALLOW, Posture.NONE)
...
def test_enforce_override_denies() -> None:
    tool = ResolvedTool(
        name="danger", server_name="fs", family="fs", posture=Posture.READ_ONLY
    )
    profile = ProfileConfig(
        name="dev",
        tools={"fs": Posture.READ_WRITE},
...
def test_enforce_override_allow_within_ceiling_still_allows() -> None:
    tool = ResolvedTool(
        name="read_special", server_name="fs", family="fs", posture=Posture.READ_ONLY
    )
    profile = ProfileConfig(
        name="dev",
        tools={"fs": Posture.READ_ONLY},
    ...
    r = enforce("read_special", tool, profile, ALLOW, Posture.NONE)
    assert r.verdict == EnforcementVerdict.ALLOW
