...
Decision: **GATE-OPEN-ALLOWED** (with residual non-blocking SKIPPED probes documented)
...
**Execution Context:**
- Worktree: `.vectl/worktrees/reclose.verify.refresh_registers`
...
- Fresh: `reclose.verify.refresh_registers` (2026-04-07)
...
|---|---|---|---|---|
| R13 | **PROVEN** | **PROVEN** | test_r13_lock_released_before_lock_acquire_await PASS; test_r13_runtime_lock_state_during_network_await PASS; test_r13_lock_hold_scope_structure_proof PASS | ✅ YES |
...
### R13 Dedicated Witness (Fresh Runtime Witness - 2026-04-07)
...
| R13 | test_r13_runtime_lock_state_during_network_await | PASS | 2026-04-07 14:03:XX |
...
| R13 | RESOLVED (structural evidence) | **PROVEN** (runtime witness) | Fresh probe run now includes the dedicated runtime witness as PASS |
...
### R13 Basis
- **PROVEN** via fresh runtime witness: test_r13_lock_hold_scope_structure_proof (PASS), test_r13_lock_released_before_lock_acquire_await (PASS), and test_r13_runtime_lock_state_during_network_await (PASS)
- The synchronized blocker basis now cites the same PASS-backed closure text instead of stale blocker-open wording.
...
- All three behavioral blocker families (`R13`, `R42-CONFIG-REMOVE-INFLIGHT`, `R42-DISCONNECT-UNDER-RECOVERY`) are dispositioned with fresh runtime witness PASS evidence
...
- Worktree: `.vectl/worktrees/reclose.verify.refresh_registers`
- Test suite result: 14 passed, 1 skipped (acceptable SKIPPED probe documented)
...
1. R13: Fresh runtime witness PASSES, all three dedicated probes confirmed
...
**None.** The authoritative blocker/gate artifacts are internally consistent with fresh execution evidence:

1. `evidence/normalized_blocker_basis.md` now mirrors `R13`, `R42-CONFIG-REMOVE-INFLIGHT`, and `R42-DISCONNECT-UNDER-RECOVERY` as `PROVEN-2026-04-07`
2. `evidence/behavioral_proof_register.md` reflects fresh test execution (14 passed, 1 skipped)
3. `evidence/runtime_uncertainty_register.md` shows PROVEN status for `R13` and both distinct R42 families with fresh runtime witness
4. `evidence/gate_review_report.md` confirms gate_open_allowed with aligned artifact references

---

## Provenance vs Disposition Check

**PASS**: Provenance is not used as a softening disposition in any artifact. Blocker families show explicit PROVEN/CLOSED status based on fresh runtime evidence, not mere provenance.
...
- `evidence/normalized_blocker_basis.md` - blocker-family rows synchronized from blocking carry rows to PASS-backed PROVEN rows; gate rule updated to `gate_open_allowed=true`
